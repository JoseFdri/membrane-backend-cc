...
const emitServerEvent = async (event: string): Promise<boolean> => await new Promise((resolve) => {
...
const closeConnection = async (): Promise<[boolean, boolean]> => {
  return await Promise.all([wsClose(), emitServerEvent('close')])
}

const openConnection = async (): Promise<[boolean, boolean]> => {
  return await Promise.all([wsConnect(), emitServerEvent('open')])
}
beforeEach(async () => {
  app = await setupApp()
